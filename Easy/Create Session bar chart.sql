...
with sessCTE(id, duration, bin) as (
SELECT sessoin_id, duration,
(CASE 
	WHEN FLOOR(duration/60) > 15 THEN "15 minutes or more"
    WHEN FLOOR(duration/60) > 10 THEN "[10-15>"
    WHEN FLOOR(duration/60) > 5 THEN  "[5-10>"
    WHEN FLOOR(duration/60) > 0 THEN "[0-5>"
    ELSE 0
END) AS bins
FROM Sessions)
